...
## 8. Behavioral Guidelines（通用 LLM 行为约束）
...
## 9. Parallel Temporary Workers

For standards checks, architecture boundary review, PR readiness review, or module contract audits, use:

`docs/superpowers/skills/auto-yolo-boundary-review/SKILL.md`

The project lead must assign disjoint ownership before anyone edits files. Review-only work is preferred for temporary workers unless a clear write scope is provided.
...
- unrelated local files such as `notes/` and model weights

Temporary workers must return findings with file and line references, checks run, and residual risk. They must stop and ask before changing scope, introducing dependencies, touching `legacy/`, or editing another worker's files.
