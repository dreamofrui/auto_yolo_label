...
from core.label_inspector import GetRunTreeConfig, LabelInspector, ListRunsConfig
...
def test_label_inspector_worker_lists_runs_and_updates_task(tmp_path: Path) -> None:
    ...
    site = tmp_path / "site"
    (site / ".autolabeler" / "inference_results" / "run_20260513_104000").mkdir(parents=True)
    registry = TaskRegistry(tmp_path / "tasks")

    outcome = LabelInspectorWorker(registry=registry).list_runs(ListRunsConfig(site_folder=site))
