...
def write_xml(
    ...
    *,
    ...
    for item in objects:
        object_fragments.append(object_template.format(**item))
    object_xml = "".join(object_fragments)
    jpg_path = path.with_suffix(".jpg")
    xml = f"""
<annotation>
  <folder>site</folder>
  <filename>{jpg_path.name}</filename>
  <path>{jpg_path}</path>
...
def test_txt_to_xml_converts_single_file_with_explicit_classes(tmp_path: Path) -> None:
    ...
    write_txt(
        folder / "labels" / "IMG_001.txt", ["0 0.500000 0.500000 0.500000 0.500000"]
    )
...
def test_txt_to_xml_recursive_false_only_top_level(tmp_path: Path) -> None:
    ...
    write_txt(
        folder / "nested" / "inner.txt", ["0 0.500000 0.500000 0.500000 0.500000"]
    )
...
def test_txt_to_xml_class_id_out_of_range_records_error_and_continues(
    tmp_path: Path,
) -> None:
...
def test_txt_to_xml_delete_source_with_backup_preserves_relative_path(
    tmp_path: Path,
) -> None:
...
def test_txt_to_xml_missing_classes_raises_convert_classes_not_found(
    tmp_path: Path,
) -> None:
...
def test_txt_to_xml_missing_folder_raises_convert_folder_not_found(
    tmp_path: Path,
) -> None:
    """Missing folders raise the dedicated folder error."""
    with pytest.raises(ConvertFolderNotFoundError):
        Converter().txt_to_xml(
            TxtToXmlConfig(folder=tmp_path / "missing", classes=["cat"])
        )
...
def test_xml_to_txt_unknown_class_raises_convert_xml_parse(tmp_path: Path) -> None:
    ...
    with pytest.raises(ConvertXmlParseError):
        Converter().xml_to_txt(
            XmlToTxtConfig(
                xml_path=xml_path, classes=["cat"], output_path=tmp_path / "out.txt"
            ),
        )
