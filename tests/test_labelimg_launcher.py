...
class FakeRunner:
    ...
    def __init__(self) -> None:
        ...
        self.python_result = FakeCompletedProcess(returncode=0, stdout="Python 3.11.14\n")
        self.labelimg_result = FakeCompletedProcess(returncode=0, stdout="labelImg 1.8.6\n")
...
def test_labelimg_config_defaults_label_and_classes_to_none(tmp_path: Path) -> None:
    ...
    config = LabelImgConfig(python_path=tmp_path / "python.exe", image_dir=tmp_path / "images")

    assert config.label_dir is None
    assert config.classes_file is None


def test_validate_returns_versions_when_python_and_labelimg_available(tmp_path: Path) -> None:
    """validate returns valid result with Python and LabelImg versions."""
    python_path = make_python(tmp_path / "python.exe")
    runner = FakeRunner()

    result = LabelImgLauncher(runner=runner).validate(LabelImgValidateConfig(python_path=python_path))
...
def test_validate_missing_python_returns_invalid_result(tmp_path: Path) -> None:
    ...
    result = LabelImgLauncher(runner=FakeRunner()).validate(LabelImgValidateConfig(python_path=missing))
...
def test_validate_labelimg_missing_returns_invalid_result(tmp_path: Path) -> None:
    ...
    python_path = make_python(tmp_path / "python.exe")
    runner = FakeRunner()
    runner.labelimg_result = FakeCompletedProcess(returncode=1, stderr="No module named labelImg")
...
def test_launch_missing_image_dir_raises_launch_error(tmp_path: Path) -> None:
    ...
    with pytest.raises(LabelImgLaunchError) as exc_info:
        LabelImgLauncher(runner=FakeRunner()).launch(
            LabelImgConfig(python_path=python_path, image_dir=tmp_path / "missing-images")
        )

    assert exc_info.value.code == ErrorCode.LABELIMG_LAUNCH


def test_launch_uses_image_dir_as_default_label_dir_and_auto_classes_file(tmp_path: Path) -> None:
    ...
    assert runner.popen_calls == [
        [str(python_path), "-m", "labelImg", str(image_dir), str(classes_file), str(image_dir)]
    ]
...
def test_launch_uses_explicit_label_dir_and_classes_file(tmp_path: Path) -> None:
    ...
    assert runner.popen_calls == [
        [str(python_path), "-m", "labelImg", str(image_dir), str(classes_file), str(label_dir)]
    ]
...
def test_launch_raises_not_installed_when_validation_command_fails(tmp_path: Path) -> None:
